...
if (installedOrca && versionAtLeast(installedOrca, PLUGIN_MIN_ORCA) && !FORCE) {
  ...
  console.log(`\n   或直接 Install → Git URL 貼上 ${PLUGIN_GIT_URL}（無法一鍵更新）`);
  if (patched) {
...
const ONBOARDING_STRINGS = [
  ...
  ["I18nProvider", "title", "Dictate into any pane"],
  ...
  ["I18nProvider", "description", "Start voice dictation in any focused pane, then use the shortcut again to stop."],
  ["I18nProvider", "ctaLabel", "Install CLI & Skills"],
  ["I18nProvider", "ctaLabel", "Got it"],
  ["I18nProvider", "ctaLabel", "Set up voice dictation"],
...
async function patch() {
  try {
    ...
    const jsxRuntimeFile = jsxRuntimeFiles
      .find(f => {
    ...
    if (jsxRuntimeFile) {
      ...
      // 補丁下拉選單
      jp.patch('renderer 語言下拉選單加入繁中',
        c => c.includes('value: "zh-TW",\n\t\tlabelKey: "settings.appearance.language.traditionalChinese"'),
        /(\{\n\t\tvalue: "zh",\n\t\tlabelKey: "settings\.appearance\.language\.chinese"\n\t\},)/,
        '$1\n\t{\n\t\tvalue: "zh-TW",\n\t\tlabelKey: "settings.appearance.language.traditionalChinese"\n\t},');

      // 補丁 fallback 名稱
      jp.patch('renderer 語言名稱 fallback 加入繁中',
        c => c.includes('["zh-TW"]: "中文（繁體）"'),
        /(\["zh"\]: "中文（简体）",\n\t)/,
        '$1["zh-TW"]: "中文（繁體）",\n\t');
