...
| **Tier 3** 設定頁 | ✅ 完成 | 3,873 句（search 關鍵字 1,401 + UI 2,472）|
| **Tier 4** 外部整合 | ✅ 完成 | #25 TaskPage、#26 stats 走正規批次；#27–#31 走三層抽查 |
| **Tier 5** 長尾模組 | 🟡 約 21% | 已抽查 browser／store.slices／lib.agent（250 句）；剩約 915 句 |
| **Tier 0** 手工精修區 | ⬜ 未抽查 | 219 句，品質已達標 |

批號以「分批清單」一節為準（由 `scripts/plan-batches.js` 產生）。改動資料後重跑產生器，
批號可能位移——別把批號寫進 commit message 以外的地方。

### Tier 4 用的是「三層抽查」而非正規批次

#25、#26 照原計畫做（逐句看＋兩輪複查）。#27–#31 改用另一種方式：
先跑全檔一致性稽核清到 0 組，再把該範圍的字串依長度分三層掃過。

  ≤6 字   829 句 → 22 句錯誤（特工產生、經過、分頁、已略過）
  7–20 字 554 句 → 18 句錯誤（恩典、移動模擬器、幫助）
  >20 字  188 句 → 20 句錯誤（路徑被翻譯、核準錯字、語意破碎）

兩種方式各有盲點，不能互相取代：

- **一致性稽核只找「分歧」。** 一個詞若從頭到尾都譯錯就完全看不到。
  `terminal→CLI` 那 70 句之所以被抓到，是因為有些地方寫「終端機」形成對比；
  `revoke→復原` 的 17 句全部一致地錯，稽核顯示 0 組。
- **三層抽查只看得到「讀起來不對」的。** 長度比例異常、佔位符缺漏、
  同一原文譯法漂移這些，靠讀是讀不出來的。

所以順序是：先用工具清掉可機械偵測的，再用眼睛掃剩下的。

### 錯誤密度隨字串長度下降，但難度上升

短標籤一眼就看出「特工產生」不對；長句裡的「5 個活動樹」得對照原文
才知道憑空多了一個字。**先掃短的**，那裡的錯誤最多也最容易判斷。

## 術語鎖定表

單一真實來源：`~/.claude/skills/tw-translate/data/domains/lock.csv`（321 條）
